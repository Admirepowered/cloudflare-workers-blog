...
const OPT = {
    ...
    "cacheZoneId":"",//清理缓存用 cf区域 ID
    "cacheToken":"",//清理缓存用 cf API token
	"visitor": true,
    "pageSize" : 5,//每页文章数
    "recentlySize" : 6,//最近文章数
    "readMoreLength":150,//阅读更多截取长度	
    "cacheTime" : 60*1, //网页缓存时长(秒),建议=文章更新频率
    "themeURL" : "https://raw.githubusercontent.com/Admirepowered/cloudflare-workers-blog/master/themes/default2.0/", // 模板地址,以 "/"" 结尾
    "html404" : `<b>404</b>`,//404页面代码
    "codeBeforHead":`<style>
        #waline-recent img {max-width: 100%;}
        #waline-recent li {border: thick double #32a1ce;}
    </style>
    <script src='//cdn.jsdelivr.net/npm/@waline/client/dist/Waline.min.js'></script>`,//其他代码,显示在</head>前
    "codeBeforBody":`<script>
            //这一段是初始化Waline的,visitor: true记得开启
            new Waline({
                el:'#waline',
                serverURL: 'https://blog.admirecn.de',
                avatar:'monsterid',
                recordIP:false,
                requiredFields:['nick'],
                visitor: true,
        ...
        </script>`,//其他代码,显示在</body>前
    "commentCode":`
    <div id="waline"  name="waline" class="comments-area"></div>
`,//评论区代码
    "widgetOther":`<div id="linkcat-0" class="widget widget_links">
        <h3 class="widget-title">
            <span>最近评论</span></h3>
        <div id="waline-recent"></div>

    </div>`,//20201224新增参数,用于右侧 小部件扩展
    "otherCodeA":`阅读次数:`,//其他参数A,可设置为 "阅读次数:"四个大字
    "otherCodeB":``,//其他参数A
    "otherCodeC":``,//其他参数A
    "otherCodeD":``,//其他参数A
    "otherCodeE":``,//其他参数A
...
async function handle(e){
	//const reqBody =await JSON.stringify(e.text());
	//const retBody = `The request body sent in was ${reqBody}`;
  //return new Response(retBody);
	
	const { pathname} = new URL(e.url);
	var path=new URL(e.url);
	var js=JSON.stringify({method,url,a,pathname,path});
	var ds=JSON.parse(js);
	var kk=ds.path;
	if (pathname.startsWith("/cdn")) {
		var url="https://raw.githubusercontent.com/Admirepowered/"+pathname
		var a =fetch(url);
	...
	if (pathname.startsWith("/comment")) {
		...
		var url="https://wanli-6k3kgns3d-admirepowered.vercel.app/"+qury;
